...
@Slf4j
public class Main {
        final static long maxCallCount = (long) 1e6;
        long seed = 0;



        public static void benchmark() throws AlgorithmUnknownException, NoBinsException {
            ...
            for (int sampleCount: sampleCounts) {
                ...
                for (int binCount : binCounts){
                    ...
                    partition.setCallLimit(Main.maxCallCount);
                    ...
                    // Combined
                    {
                        ...
                        long start = System.currentTimeMillis();
                        partition.compute(Partition.Algorithm.GREEDY_DSTREE);
                        long finish = System.currentTimeMillis();
                        long timeElapsed = finish - start;
                        log.info("\tCalls: " + partition.getCallCount() + "\tBottomCalls: " + partition.getBottomCalls());
                        row.add("|");
        ...
        public static void main(String[] args) throws AlgorithmUnknownException, NoBinsException {
            Main.benchmark();
        }
    }
